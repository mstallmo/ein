...
| `/config` + `Enter` | Open `~/.ein/config.json` in `$EDITOR` |
...
The TUI connects in the background immediately on startup. While disconnected, a red `●` icon and animated spinner appear in the conversation pane. The TUI reconnects automatically every 3 seconds. If the server goes away mid-session, an error message is shown and the TUI resumes connecting in the background — the session picker reappears on the next successful reconnect.
...
`ListSessions` returns a list of `SessionSummary` records (newest-first), each containing the session ID, creation timestamp, a preview of the first user message, and the stored `SessionConfig` JSON needed to reconstruct the session on resume.
...
| `connection.rs` | `connection_manager` — background reconnect loop, `ListSessions` handshake, config file watcher |
